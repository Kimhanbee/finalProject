...
document.addEventListener("DOMContentLoaded", () => {
    ...
    $(document).on('click', '.edit-btn', function () {
        ...
        $tr.find('[data-key]').each(function () {
			...
			//진취,사회는 select박스
          // 진취형, 사회형에 대해 select 박스 생성
			if (key === 'cocoStts') {
			    let selectHtml = `<select class="form-select" data-origin="${text}">`;
			    ...
			    // selected 값 찾기
			    const matchedValue = hardcodedOptions.find(option => option.label === text)?.value;
    ...
    $(document).on("click", ".add-save-btn", function () {
		...
		axios.post(`${cp}/jobtest/create`, { jobTestType, jobTestText })
            ...
            `;
				...
				swal("실패", "문제 추가 중 문제가 발생했습니다.", "error");
			});
